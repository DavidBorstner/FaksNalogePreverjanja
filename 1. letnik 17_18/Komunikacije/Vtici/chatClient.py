...
def message_receiver():
    while True:
        ...
        if len(msg_received) > 0:  # ce obstaja sporocilo
            print("["+ IME + " " + cas + "]: " + msg_received)  # izpisi

while (True):
    text = input("Vpiši svoje ime: ")
    if (text != ""):
        IME = text
        break
...
try:  # Strežniku pošlje uporabniško ime
    msg_send = IME
    cas = strftime("%H:%M:%S", localtime())
    send_message(sock, cas + ";PRIJAVA;" + msg_send)
